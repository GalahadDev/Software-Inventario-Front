...
import { DollarSign, CreditCard, Truck, Package, MapPin, ClipboardList, Calendar } from "lucide-react";
import { SearchDate } from "../../ReusableComponents/SearchDate";
import { StatusColor } from "app/types";
...
const [totalMonto, setTotalMonto] = useState(0);
  ...
  // Elimina cualquier parte no deseada al final de la URL, como parámetros adicionales
  const baseUrl = url.split('?')[0];  // Toma todo antes de cualquier parámetro
  return baseUrl;
};
...
const handleSendToWhatsApp = (pedido: Pedido) => {
  ...
  // Crear el mensaje con la URL de la imagen limpia
  const mensaje = `Pedido: ${pedido.Nombre}\nDescripción: ${pedido.Descripcion}\nObservaciones: ${pedido.Observaciones}\nImagen: ${imagenLimpia}`;
  
  // Codificar el mensaje para asegurarse de que se pueda enviar correctamente
  const mensajeCodificado = encodeURIComponent(mensaje);

  // URL del grupo de WhatsApp (sin cambios)
