...
const PedidosPage = () => {
  ...
  // ... (efectos para actualizar pedidos y nuevos pedidos sin cambios)

  // --- Modificación en el cálculo del total de montos ---
  const calcularTotalMonto = () => {
    ...
    // Convertir las fechas del rango a Chile
    ...
    const pedidosFiltrados = pedidosNoPagados.filter((pedido) => {
      // Convertir la fecha del pedido a Chile
      const pedidoFechaChile = toChileDate(new Date(pedido.FechaCreacion));
      return (
        pedidoFechaChile >= fechaInicioChile &&
        pedidoFechaChile <= fechaTerminoChile &&
  ...
  // --- Modificación en el filtrado de pedidos ---
  const filteredPedidos = useMemo(() => {
    ...
    return sortedPedidos.filter((pedido) => {
      // Convertir la fecha del pedido a Chile
      const fechaCreacion = toChileDate(new Date(pedido.FechaCreacion));
      if (isNaN(fechaCreacion.getTime())) return false;

      // Convertir startDate y endDate a Chile (si existen)
      ...
      const isInRange = fechaInicioChile && fechaTerminoChile
        ? (fechaCreacion >= fechaInicioChile && fechaCreacion <= fechaTerminoChile)
        : true;
  ...
  // El resto de la lógica (handleCardClick, handleSendToWhatsApp, updateMonto, getStatusColor, etc.) se mantiene igual
